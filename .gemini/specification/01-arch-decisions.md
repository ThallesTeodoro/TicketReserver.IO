...
*   **Decisão:** O processamento da tabela de Outbox será extraído para um console application .NET isolado (`TR.HW.OutboxRelay`), empacotado em um container Docker independente e escalado de forma autônoma no Kubernetes.
...
*   **Decisão:**
    *   **Borda (TR.API.Reservation):** Camada de cache distribuído em Redis com TTL de 24 horas usando o comando atômico `SET NX`.
        *   **Padrão de Chave:** `idempotency:reservation:{key}`.
    *   **Núcleo (TR.HW.Payment):** Constraint de chave única (`UNIQUE INDEX`) no Postgres baseada no `ReservationId`.
...
*   **Decisão:** Cada microsserviço reagirá autonomamente a eventos específicos no Apache Kafka. O rollback de estado será feito via publicação de eventos de compensação (ex: `payment.refused`).
...
flowchart TD
    ...
    Ingress --> API_Res[TR.API.Reservation]
    API_Res <--> Redis[(Redis \nIdempotency)]
    ...
    subgraph SVC_RESERVA [Microserviço de Reserva]
        ...
        Outbox_Relay[TR.HW.OutboxRelay] -- "3. Lê pendências" --> DB_Res
    end
    ...
    subgraph SVC_PAGAMENTO [Microserviço de Pagamento]
        Worker_Pag[TR.HW.Payment] -- "5. Consome" --> Kafka
    ...
    subgraph SVC_TICKETING [Microserviço de Emissão]
        Worker_Ticket[TR.HW.Ticketing] -- "8. Consome" --> Kafka
